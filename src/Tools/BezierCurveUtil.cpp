...
namespace sg {
    ...
    std::queue<sf::Vector2f> BezierCurveUtil::computeBezierCurveDegree1(const std::vector<sf::Vector2f> &points, float nb_points)
    {
    ...
    std::queue<sf::Vector2f> BezierCurveUtil::computeBezierCurveDegree2(const std::vector<sf::Vector2f> &points, float nb_points)
    {
    ...
    std::queue<sf::Vector2f> BezierCurveUtil::computeBezierCurveDegree3(const std::vector<sf::Vector2f> &points, float nb_points)
    {
        const sf::Vector2f &p0 = points[0];
        const sf::Vector2f &p1 = points[1];
        const sf::Vector2f &p2 = points[2];
    ...
    std::queue<sf::Vector2f> BezierCurveUtil::computeBezierCurveDegreeN(const std::vector<sf::Vector2f> &points, float nb_points)
    {
        std::queue<sf::Vector2f> computedPoints;
        for(float i = 0.0f; i < 1.0f; i += 1.f / nb_points)
        {
    ...
    sf::Vector2f BezierCurveUtil::casteljau(const std::vector<sf::Vector2f> &points, float param)
    {
        if (points.size() == 1) {
            return points[0];
        }
